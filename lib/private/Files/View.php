	...
	protected function emit_file_hooks_pre($exists, $path, &$run) {
		...
		if (!$exists) {
			...
			if ($run) {
				...
				$this->eventDispatcher->dispatch('file.create.allowed', $event);
			}
		} else {
			...
			if ($run) {
				$event->setArgument('run', $run);
				$this->eventDispatcher->dispatch('file.update.allowed', $event);
			}
		}
		...
		if ($run) {
			$event->setArgument('run', $run);
			$this->eventDispatcher->dispatch('file.write.allowed', $event);
		}
	}
